...
#include <pthread.h> // Добавлено для поддержки многопоточности
...
bakery_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union
	{
		struct ServiceRequest get_service_1_arg;
	} argument;
	union
	{
		struct Ticket get_ticket_1_res;
		struct ServiceResponse get_service_1_res;
	} result;
	bool_t retval;
	...
	case GET_SERVICE:
		_xdr_argument = (xdrproc_t)xdr_ServiceRequest;
		_xdr_result = (xdrproc_t)xdr_ServiceResponse;
