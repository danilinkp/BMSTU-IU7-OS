...
void read_environ(FILE *output, const char *path, const char *desc)
{
...
void read_file(FILE *output, const char *path, const char *desc)
{
    ...
    while ((n = fread(buffer, 1, sizeof(buffer), file)) > 0)
    {
        ...
        {
            if (buffer[i] == '\0')
                fputc('\n', output); // Заменяем разделитель на перенос строки
            else
                fputc(buffer[i], output);
        }
    }
    fputc('\n', output); // Финальный перенос для красоты
    fclose(file);
}
...
int main(int argc, char *argv[])
{
    ...
    read_environ(output, path, "Process Environment Variables");
